...
- BAN exact technical details from ALL fantasy sections: no numbers/counts, no chain names, no subgraph hashes/ids, no function names, no token symbols. Convey scale and character with words (few/many, near/far, still/restless), not figures.
...
2. "whatItCanDo" (FANTASY) — Metaphor about what the beast is literally SEEN doing in the lands, grounded in ABI/data (e.g. "in the markets it is watched swapping treasures" only if swap functions/entities exist). NO function names, NO hashes, NO counts, NO codes. "It weaves swaps across many markets" not "it can swap tokens".
3. "ecosystemTracking" (FANTASY) — The heart of this tale: the SUBGRAPH KINGDOM. Describe the terrain and who watches this beast, grounded ONLY in the data. Weave the essence into soft, suggestive language — the number of lands, the far realms, the many watchers — WITHOUT exact counts, codes, hashes, chain names, or subgraph ids. E.g. "across distant realms, many watchers file their own scrolls about it" — not "5 territories, base and zksync-era, QmXTBmwBm1NF". The reader should sense the scale (large/small, still/restless) from the prose, not from numbers. NEVER name tokens/contracts not in the data. Always note the map is partial.
4. "riskyBusiness" (FANTASY) — Metaphor about perils the watchers whisper about, grounded ONLY in ABI risk funcs if present (pause/blacklist/deprecate...). If none: "the watchers see no traps". NO function names, NO hashes, NO counts. Describe perils in plain words ("some say the guardians can lock the vault") while the specific function stays unspoken in the prose. Never invent risks.
